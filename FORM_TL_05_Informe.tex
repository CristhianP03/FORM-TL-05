...
A partir de la matriz anterior, el equipo pondera los criterios según su relevancia para un sistema de tickets que debe permanecer disponible de forma continua y que requiere un despliegue distribuido tolerante a fallos entre zonas geográficas. En ese sentido, el criterio de \emph{SLA de disponibilidad} y el de \emph{facilidad de integración con Kubernetes} adquieren un peso mayor que el costo puntual, dado que una interrupción del servicio de soporte técnico tiene un costo reputacional para el ISP que excede la diferencia de precio entre proveedores.
...
El equipo selecciona Google Cloud Platform como proveedor de nube para alojar el sistema de gestión de soporte técnico ISP. La decisión se sustenta en cuatro elementos concretos derivados de la matriz de la Sección 4. En primer lugar, Google Kubernetes Engine (GKE) es, de los tres servicios comparados, el de mayor madurez operativa, dado que Google es el creador original del proyecto Kubernetes; esto reduce la curva de aprendizaje del equipo y el riesgo de errores de configuración durante el despliegue del clúster que orquesta los microservicios del sistema de tickets. En segundo lugar, GKE ofrece un SLA de 99.95\,\% en su modalidad regional o Autopilot, equivalente al mejor nivel disponible tanto en AWS como en Azure, por lo que no se sacrifica disponibilidad contractual al elegir este proveedor. En tercer lugar, Cloud Spanner permite implementar una base de datos relacional distribuida globalmente de forma nativa, sin que el equipo deba configurar manualmente los mecanismos de replicación entre las zonas Norte, Centro y Sur del sistema, lo que simplifica considerablemente la arquitectura frente a las alternativas de AWS y Azure. Finalmente, GCP dispone de una región propia en Sudamérica (\texttt{southamerica-east1}, São Paulo), con una cercanía geográfica a Ecuador comparable a la que ofrecen AWS y Azure, por lo que este criterio no representa una desventaja frente a los otros dos proveedores. El equipo reconoce como riesgo aceptado la dependencia de un único proveedor (\textit{vendor lock-in}), particularmente relevante si se adopta Cloud Spanner, cuya API no es directamente portable a otros proveedores; este riesgo se documenta formalmente en el ADR-001.
